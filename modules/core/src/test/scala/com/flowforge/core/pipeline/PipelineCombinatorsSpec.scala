...
class PipelineCombinatorsSpec extends AnyFunSuite with Matchers {
  ...
  test("sequence composes steps in order", How) {
  ...
  test("parallel runs both and combines", How) {
  ...
  test("conditional chooses correct branch", How) {
  ...
  test("retry re-executes underlying pipeline", How) {
  ...
  test("batch groups and flattens results", How) {
