...
/**
 * Broad, fast smoke-style tests that intentionally exercise many branches across core types and syntax.
 * This focuses on behavior already covered by unit tests but drives additional code paths for coverage.
 */
class CoreFastCoverageSpec extends AnyFunSuite with Matchers {
  ...
  test("DataFormat and CompressionType properties are consistent") {
    val fmts = List(DataFormat.Parquet, DataFormat.Avro, DataFormat.CSV, DataFormat.JSON, DataFormat.JSONL, DataFormat.ORC, DataFormat.Delta)
    fmts.foreach { f =>
    ...
    val comps = List(CompressionType.Snappy, CompressionType.Gzip, CompressionType.Bzip2, CompressionType.LZ4, CompressionType.Deflate, CompressionType.Zlib)
  ...
  test("Effect syntax fast path (IO)") {
    implicit val logger: com.flowforge.core.logging.CoreLogger[IO] = new com.flowforge.core.logging.CoreLogger[IO] {
      def info(msg: String)  = IO.unit
      def warn(msg: String)  = IO.unit
      def error(msg: String) = IO.unit
    }
    implicit val es: com.flowforge.core.algebra.EffectSystem[IO] = com.flowforge.core.instances.EffectInstances.catsEffectSystemInstance
    val out = IO.pure(1)
