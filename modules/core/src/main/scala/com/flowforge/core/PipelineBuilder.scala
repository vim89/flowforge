...
case class PipelineBuilder[S <: BuilderState, F[_]: EffectSystem, In, Out] private (
  ...
  /**
   * Add typed source with explicit contract and policy.
   * This is the ONLY way to add sources - no untyped escape hatches.
   * 
  ...
  def addTypedSource[R, P <: SchemaPolicy](
    ...
    PipelineBuilder[HasSource with HasContract, F, Unit, R](
      name, description, stages :+ stage, config
    )
  }

  /**
   * Add transformation stage.
   * Advances phantom state: HasSource with HasContract -> HasSource with HasContract with HasTransform
   */
  def addTransform[C](
    transform: Out => F[C]
  )(implicit 
  ...
  /**
   * Add typed sink with explicit contract and policy.
   * This is the ONLY way to add sinks - no untyped escape hatches.
   * 
  ...
  /**
   * Build pipeline - ONLY available when all required stages are present.
   * This is the key compile-time guarantee: incomplete pipelines cannot be built.
   */
  def build()(implicit 
...
object PipelineBuilder {
   ...
   * Starts with Empty phantom state - must add source, transform, and sink to build.
   */
