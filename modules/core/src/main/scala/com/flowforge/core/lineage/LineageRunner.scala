      ...
      case st :: tail =>
        F.flatMap(emitter.emitJobStart("flowforge", s"stage-$idx", java.util.UUID.randomUUID.toString)) { _ =>
          F.flatMap(F.attempt(kAny(st.execute).run(in))) {
            case Right(out) =>
              F.flatMap(emitter.emitJobComplete("flowforge", s"stage-$idx", "run"))(_ =>
                loop(idx + 1, out, tail),
              )
            case Left(e) =>
              F.flatMap(emitter.emitJobFail("flowforge", s"stage-$idx", "run", e.getMessage))(_ =>
                F.raiseError(e),
              )
          }
        }
