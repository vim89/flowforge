...
final class InMemoryDataAlgebra[F[_]](implicit F: EffectSystem[F]) extends DataAlgebra[F] {
  ...
  override def read[A: DataDecoder](source: DataSource): F[Dataset[A]] = source match {
    ...
    case _ => F.raiseError(new UnsupportedOperationException("Unsupported DataSource for in-memory algebra"))
  }
  ...
  override def write[A: DataEncoder](
    ...
    case _ => F.raiseError(new UnsupportedOperationException("Unsupported sink for in-memory algebra"))
  }
  ...
  override def union[A](left: Dataset[A], right: Dataset[A]): Dataset[A] =
    SimpleDataset(left.data ++ right.data, left.schema, left.metadata.copy(recordCount = (left.data.size + right.data.size).toLong))
  ...
  override def take[A](dataset: Dataset[A], n: Int): Dataset[A] =
    SimpleDataset(dataset.data.take(n), dataset.schema, dataset.metadata.copy(recordCount = math.min(dataset.data.size, n).toLong))

  override def drop[A](dataset: Dataset[A], n: Int): Dataset[A] =
    SimpleDataset(dataset.data.drop(n), dataset.schema, dataset.metadata.copy(recordCount = math.max(0, dataset.data.size - n).toLong))
  ...
  override def transformPipeline[A, B: DataEncoder](
    ...
    val composed = transformations.reduceLeft { (f, g) => a => F.flatMap(f(a))(b => F.map(g(a))(_ => b)) }
  ...
  ): F[Dataset[B]] = F.raiseError(new NotImplementedError("Schema migration not implemented for in-memory algebra"))

  override def compareSchemas(
    schema1: DataSchema,
    schema2: DataSchema
  ): F[SchemaCompatibilityReport] =
    F.pure(SchemaCompatibilityReport(schema1.fieldNames.toSet == schema2.fieldNames.toSet, Nil, Nil))
  ...
  override def recordLineage[A](
    ...
    F.pure(LineageRecord(java.util.UUID.randomUUID().toString, dataset.metadata.source.getOrElse(LocalDataSource("unknown", DataFormat.JSON)), None, operation, context, List(dataset.schema), Some(dataset.schema)))
  ...
  override def runQualityChecks[A](
    ...
    checks.toList.zipWithIndex.map { case (_, idx) => QualityCheckResult(s"check_$idx", passed = true, message = "ok", score = 1.0) }
  )
