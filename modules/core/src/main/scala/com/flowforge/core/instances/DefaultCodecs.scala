...
object DefaultCodecs {
  ...
  implicit val stringDecoder: DataDecoder[String] = new DataDecoder[String] {
    ...
    def decodeWithEvolution(encodedData: EncodedData, format: DataFormat, targetSchema: DataSchema) =
      Right(new String(encodedData.data, "UTF-8"))
  ...
  implicit val jsonEncoder: DataEncoder[Json] = new DataEncoder[Json] {
    def encode(data: Json, format: DataFormat) = Right(EncodedData(data.noSpaces.getBytes("UTF-8"), format))
    def schema(format: DataFormat): DataSchema = DataSchema.builder.addField("json", DataType.String).build
  ...
  implicit val jsonDecoder: DataDecoder[Json] = new DataDecoder[Json] {
    def decode(encodedData: EncodedData, format: DataFormat) =
      parser.parse(new String(encodedData.data, "UTF-8")).left.map(err => CorruptedData(err.getMessage))
    def validateSchema(encodedData: EncodedData, expectedSchema: DataSchema) = Right(())
    def decodeWithEvolution(encodedData: EncodedData, format: DataFormat, targetSchema: DataSchema) =
  ...
  implicit val mapStringStringEncoder: DataEncoder[Map[String, String]] = new DataEncoder[Map[String, String]] {
    def encode(data: Map[String, String], format: DataFormat) = format match {
      case DataFormat.JSON | DataFormat.JSONL =>
        Right(EncodedData(Json.obj(data.toSeq.map { case (k, v) => (k, Json.fromString(v)) }: _*).noSpaces.getBytes("UTF-8"), format))
      case DataFormat.CSV =>
    ...
    def optimizationHints(data: Map[String, String], format: DataFormat): EncodingHints = EncodingHints.default
  }

  implicit val mapStringStringDecoder: DataDecoder[Map[String, String]] = new DataDecoder[Map[String, String]] {
    def decode(encodedData: EncodedData, format: DataFormat) = format match {
      case DataFormat.JSON | DataFormat.JSONL =>
        parser
          .parse(new String(encodedData.data, "UTF-8"))
          .left.map(err => CorruptedData(err.message))
          .flatMap { j =>
            j.asObject
