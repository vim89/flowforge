 ...
 * 
 * This module provides type class instances for all FlowForge data types,
 * ensuring they integrate seamlessly with the Cats ecosystem and provide
...
object DataInstances {
   ...
   */
  implicit val stringDataEncoder: DataAlgebra.DataEncoder[String] = new DataAlgebra.DataEncoder[String] {
  ...
  implicit val doubleDataEncoder: DataAlgebra.DataEncoder[Double] = new DataAlgebra.DataEncoder[Double] {
  ...
  implicit val booleanDataEncoder: DataAlgebra.DataEncoder[Boolean] = new DataAlgebra.DataEncoder[Boolean] {
  ...
  implicit val instantDataEncoder: DataAlgebra.DataEncoder[Instant] = new DataAlgebra.DataEncoder[Instant] {
  ...
  /**
   * Generic encoder for case classes (simplified - would use reflection/macros in real implementation)
   */
  implicit def caseClassEncoder[A <: Product]: DataAlgebra.DataEncoder[A] = new DataAlgebra.DataEncoder[A] {
   ...
   */
  implicit val stringDataDecoder: DataAlgebra.DataDecoder[String] = new DataAlgebra.DataDecoder[String] {
  ...
  implicit val intDataDecoder: DataAlgebra.DataDecoder[Int] = new DataAlgebra.DataDecoder[Int] {
    def decode(data: DataAlgebra.EncodedData): Either[FlowForgeError, Int] = {
      ...
      scala.util.Try(str.toInt).toEither.left.map(e => 
        FlowForgeError.DataProcessingError(s"Cannot decode '$str' as Int: ${e.getMessage}", Some(e))
      )
    }
    def expectedSchema: DataAlgebra.DataSchema = 
      DataAlgebra.DataSchema("int", Map("type" -> "integer"), List("non-null"))
  ...
  implicit val longDataDecoder: DataAlgebra.DataDecoder[Long] = new DataAlgebra.DataDecoder[Long] {
    def decode(data: DataAlgebra.EncodedData): Either[FlowForgeError, Long] = {
      val str = new String(data.bytes, "UTF-8")
      scala.util.Try(str.toLong).toEither.left.map(e => 
        FlowForgeError.DataProcessingError(s"Cannot decode '$str' as Long: ${e.getMessage}", Some(e))
      )
    }
    def expectedSchema: DataAlgebra.DataSchema = 
      DataAlgebra.DataSchema("long", Map("type" -> "long"), List("non-null"))
  ...
  implicit val doubleDataDecoder: DataAlgebra.DataDecoder[Double] = new DataAlgebra.DataDecoder[Double] {
    def decode(data: DataAlgebra.EncodedData): Either[FlowForgeError, Double] = {
      val str = new String(data.bytes, "UTF-8")
      scala.util.Try(str.toDouble).toEither.left.map(e => 
        FlowForgeError.DataProcessingError(s"Cannot decode '$str' as Double: ${e.getMessage}", Some(e))
      )
    }
    def expectedSchema: DataAlgebra.DataSchema = 
      DataAlgebra.DataSchema("double", Map("type" -> "double"), List("non-null"))
  ...
  implicit val booleanDataDecoder: DataAlgebra.DataDecoder[Boolean] = new DataAlgebra.DataDecoder[Boolean] {
    def decode(data: DataAlgebra.EncodedData): Either[FlowForgeError, Boolean] = {
      ...
      str.toLowerCase match {
        ...
        case _ => Left(FlowForgeError.DataProcessingError(s"Cannot decode '$str' as Boolean", None))
      }
    }
    def expectedSchema: DataAlgebra.DataSchema = 
      DataAlgebra.DataSchema("boolean", Map("type" -> "boolean"), List("non-null"))
  ...
  implicit val instantDataDecoder: DataAlgebra.DataDecoder[Instant] = new DataAlgebra.DataDecoder[Instant] {
    def decode(data: DataAlgebra.EncodedData): Either[FlowForgeError, Instant] = {
      val str = new String(data.bytes, "UTF-8")
      scala.util.Try(Instant.parse(str)).toEither.left.map(e => 
        FlowForgeError.DataProcessingError(s"Cannot decode '$str' as Instant: ${e.getMessage}", Some(e))
      )
    }
    def expectedSchema: DataAlgebra.DataSchema = 
      DataAlgebra.DataSchema("timestamp", Map("type" -> "timestamp"), List("non-null"))
   ...
   */
  implicit val validationResultApplicative: Applicative[ValidationResult] = new Applicative[ValidationResult] {
  ...
  implicit val configErrorShow: Show[ConfigError] = Show.show {
    ...
    case ConfigError.InvalidFormat(field, value, expected) => s"InvalidFormat($field=$value, expected=$expected)"
    case ConfigError.OutOfRange(field, value, min, max) => s"OutOfRange($field=$value, range=[$min, $max])"
  }
  ...
  def createMockDataAlgebra[F[_]: EffectSystem]: DataAlgebra[F] = new DataAlgebra[F] {
    ...
    def mapWithEffect[A, B: DataEncoder](dataset: Dataset[A], f: A => F[B]): F[Dataset[B]] =
      dataset.data.traverse(f).map(transformedData => 
        Dataset.fromList(transformedData, s"${dataset.id}_mapped")
      )
    ...
    def runQualityChecks[A](dataset: Dataset[A], checks: NonEmptyList[QualityCheck[A]]): F[List[QualityCheckResult]] =
      F.delay(List.empty)
    ...
    def compareSchemas(source: DataSchema, target: DataSchema): F[SchemaCompatibilityReport] =
      F.delay(SchemaCompatibilityReport(
        compatible = true,
        issues = List.empty,
        recommendations = List.empty
      ))
    
    def validateSchema[A](dataset: Dataset[A], schema: DataSchema): F[ValidatedNel[FlowForgeError, Dataset[A]]] =
   ...
   */
  def dataEncoder[A](implicit encoder: DataAlgebra.DataEncoder[A]): DataAlgebra.DataEncoder[A] = encoder
  def dataDecoder[A](implicit decoder: DataAlgebra.DataDecoder[A]): DataAlgebra.DataDecoder[A] = decoder
