...
/**
  ...
  * object Contracts {
  *   // compile‑time evidence (fails to compile if drift exists)
  *   implicit val ev: SchemaConforms[User, User, SchemaPolicy.Exact] = implicitly
  * }
  *
  * val builder = PipelineBuilder[IO]("users")
  *   .addTypedSource[User, User, SchemaPolicy.Exact]( /* source */ null, _ => IO.pure(User(1,"a@b")))
  *   .noTransform
  *   .addTypedSink[User, SchemaPolicy.Exact]( /* sink */ null, (_, _) => IO.unit)
  *
