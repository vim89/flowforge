...
object ConfigurationManagement {
    ...
    def loadTypeSafeConfig[T: ConfigDecoder](key: String): F[ValidatedNel[ConfigError, T]] =
      Sync[F].delay {
        Try {
          ...
          case Failure(ex) => cats.data.Validated.invalidNel(ConfigError.ParseError(key, ex.getMessage))
        }
      }
  ...
  implicit val flowForgeConfigDecoder: ConfigDecoder[FlowForgeConfig] = new ConfigDecoder[FlowForgeConfig] {
    def decode(config: Config, path: String): ValidatedNel[ConfigError, FlowForgeConfig] =
      ...
      cats.data.Validated.invalidNel(ConfigError.ParseError(path, "FlowForgeConfig decoding not yet implemented - placeholder for compilation"))
  }
}
