...
/**
 * Resource safety framework providing automatic resource management with bracket patterns.
 * This is a core Infrastructure Layer component that ensures all external resources
 * (connections, files, streams) are properly cleaned up.
 */
trait ResourceSafety[F[_]] {
  
  /**
   * Basic bracket pattern for resource acquisition and cleanup.
   * Ensures release is called even if use throws an exception.
   */
  def bracket[A, B](acquire: F[A])(use: A => F[B])(release: A => F[Unit]): F[B]
  
  /**
   * Advanced bracket with exit case handling.
   * Provides information about how the use operation completed.
   */
  def bracketCase[A, B](
    acquire: F[A]
  )(
...
object ResourceSafety {
   ...
   */
  private class CatsEffectResourceSafety[F[_]: Sync: MonadCancel[*[_], Throwable]] extends ResourceSafety[F] {
...
/**
 * Cloud-specific resource safety for managing cloud connections and handles.
 * Extends basic ResourceSafety with cloud provider abstractions.
 */
trait CloudResourceSafety[F[_]] extends ResourceSafety[F] {
  
  /**
   * Safe cloud connection management.
   * Automatically handles authentication, connection pooling, and cleanup.
   */
...
object CloudResourceSafety {
  ...
  private class DefaultCloudResourceSafety[F[_]: ResourceSafety: Sync] extends CloudResourceSafety[F] {
