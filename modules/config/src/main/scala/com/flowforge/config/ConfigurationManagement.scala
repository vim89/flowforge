...
/**
 * Type-safe configuration management system - replacement for CCM.
 * Provides compile-time safe configuration loading with validation.
 */
trait ConfigurationManagement[F[_]] {
  
  /**
   * Load type-safe configuration with validation.
   * Returns all validation errors if configuration is invalid.
   */
...
object ConfigurationManagement {
  ...
  private class TypesafeConfigManagement[F[_]: Sync] extends ConfigurationManagement[F] {
    ...
    override def loadTypeSafeConfig[T: ConfigDecoder](key: String): F[ValidatedNel[ConfigError, T]] = {
      Sync[F].delay {
...
object ConfigDecoder {
  ...
  implicit val finiteDurationDecoder: ConfigDecoder[FiniteDuration] = new ConfigDecoder[FiniteDuration] {
  ...
  implicit val pipelineConfigDecoder: ConfigDecoder[PipelineConfig] = new ConfigDecoder[PipelineConfig] {
    override def decode(config: Config, path: String): ValidatedNel[ConfigError, PipelineConfig] = {
  ...
  implicit val monitoringConfigDecoder: ConfigDecoder[MonitoringConfig] = new ConfigDecoder[MonitoringConfig] {
    override def decode(config: Config, path: String): ValidatedNel[ConfigError, MonitoringConfig] = {
  ...
  implicit val connectorConfigDecoder: ConfigDecoder[ConnectorConfig] = new ConfigDecoder[ConnectorConfig] {
    override def decode(config: Config, path: String): ValidatedNel[ConfigError, ConnectorConfig] = {
      ConnectorConfig().validNel // Empty for now
    }
  }
  
  implicit val flowForgeConfigDecoder: ConfigDecoder[FlowForgeConfig] = new ConfigDecoder[FlowForgeConfig] {
    override def decode(config: Config, path: String): ValidatedNel[ConfigError, FlowForgeConfig] = {
