...
object SchemaValidateCli extends IOApp.Simple {
  ...
  private val parser = {
    ...
    OParser.sequence(
      ...
      opt[String]("input").required().action((p, a) => a.copy(pathOrTable = p)).text("path or table"),
      opt[java.io.File]("expected-json").required().action((f, a) => a.copy(expectedSchemaJson = f)).text("expected schema file (Spark JSON or Avro or JSON Schema)"),
      opt[String]("expected-format").optional().action((s, a) => a.copy(expectedFormat = s.toLowerCase match {
        case "spark" | "" => ExpectedFormat.Spark
        case other          => throw new IllegalArgumentException(s"Unsupported expected-format: $other (only 'spark' supported in this build)")
      })).text("spark (default)"),
      opt[String]("master").optional().action((m, a) => a.copy(master = Some(m))).text("Spark master, e.g., local[*]")
    )
  }
  ...
  private def sparkResource(master: Option[String]): Resource[IO, SparkSession] =
    Resource.make {
      EffectSystem[IO].blocking {
        val b = SparkSession.builder().appName("ff-validate-schema").config("spark.ui.enabled", "false")
  ...
  object Canonical {
          ...
          case Some(arr) =>
            val fields = arr.flatMap { j =>
              ...
              for {
                ...
                tpe  <- c.downField("type").get[String]("type").orElse(c.get[String]("type")).toOption
    ...
    def diff(expected: Model, actual: Model): List[Diff] = {
      ...
      val common  = expMap.keySet.intersect(actMap.keySet).toList.flatMap { n =>
        ...
        val d1 = if (normalizeTypeName(e.tpe) != normalizeTypeName(a.tpe)) Some(Diff.TypeMismatch(n, e.tpe, a.tpe)) else None
        val d2 = if (e.nullable != a.nullable) Some(Diff.NullabilityMismatch(n, e.nullable, a.nullable)) else None
        List(d1, d2).flatten
      }
    ...
    def prettyDiffs(ds: List[Diff]): String = {
      val lines = ds.map {
        ...
        case Diff.NullabilityMismatch(n,e,a) => s"- Nullability mismatch for $n: expected=$e actual=$a"
      }
