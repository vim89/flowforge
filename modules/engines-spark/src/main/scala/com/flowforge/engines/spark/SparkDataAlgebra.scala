...
object SparkDataAlgebra {
   ...
   */
  /**
    * Pair of a concrete [[com.flowforge.core.algebra.DataAlgebra]] implementation and the [[org.apache.spark.sql.SparkSession]]
    * it was created with. Returned by [[createSparkDataAlgebra]] to make resource handling (e.g. closing the
    * session) explicit at call sites.
    *
    * @tparam F effect type (e.g., cats.effect.IO)
    * @param algebra concrete DataAlgebra backed by Spark
    * @param sparkSession the Spark session used to perform operations
    */
  ...
  /**
    ...
    *
    * @param sparkSession an existing SparkSession (caller controls its lifecycle)
    * @tparam F effect type with an [[com.flowforge.core.algebra.EffectSystem]] instance
    * @return the algebra paired with the session for explicit lifecycle
    */
