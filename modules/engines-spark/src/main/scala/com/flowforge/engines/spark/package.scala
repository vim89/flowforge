 ...
 *
 * ==Quick start==
 * {{@example import cats.effect.IO import org.apache.spark.sql.SparkSession import
 * com.flowforge.engines.spark.SparkDataAlgebra
 *
 * val spark = SparkSession.builder().master("local[*]").appName("ff").getOrCreate() val alg =
