...
class SparkDeltaSCD1IT extends AnyFunSuite {
  ...
  test("SCD1 upsert using Delta MERGE (opt-in)") {
    ...
    val spark = SparkSession
      .builder().appName("ff-scd1").master("local[2]")
    ...
    val cfg = CDCOperations.CDCConfig(keyColumns =
      cats.data.NonEmptyList.one(RefinedTypes.FieldName.unsafeFrom("id")),
    )
