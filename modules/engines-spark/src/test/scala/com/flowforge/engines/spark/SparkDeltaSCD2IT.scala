...
final case class S2(id: Long, v: String, ts: Long)
...
class SparkDeltaSCD2IT extends AnyFunSuite {
  ...
  test("SCD2 merge with effective_from/effective_to/is_current (opt-in)") {
    ...
    val spark  = SparkSession.builder().appName("ff-scd2").master("local[2]")
      .config("spark.sql.extensions","io.delta.sql.DeltaSparkSessionExtension")
      .config("spark.sql.catalog.spark_catalog","org.apache.spark.sql.delta.catalog.DeltaCatalog").getOrCreate()
    val t1 = System.currentTimeMillis()
    val df1 = spark.range(1, 2)
    ...
    val cfg = CDCOperations.CDCConfig(
      ...
      scd2 = Some(CDCOperations.SCD2Columns(
  ...
  test("SCD2 with composite keys and partitioning (opt-in)") {
    ...
    val spark = SparkSession.builder().appName("ff-scd2-composite").master("local[2]")
      .config("spark.sql.extensions","io.delta.sql.DeltaSparkSessionExtension")
      .config("spark.sql.catalog.spark_catalog","org.apache.spark.sql.delta.catalog.DeltaCatalog").getOrCreate()
    ...
    val cfg = CDCOperations.CDCConfig(
      ...
      partition = Some(CDCOperations.PartitionStrategy(List(FieldName.unsafeFrom("id"), FieldName.unsafeFrom("cat"))))
    )
    ...
    assert(tgt2.filter("is_current = true").count() == 1)
    assert(tgt2.filter("is_current = false").count() >= 1)
    val partCols = spark.sql(s"DESCRIBE DETAIL delta.`$tgtDir`").selectExpr("partitionColumns").head().getSeq[String](0)
