...
/**
 * Realistic Spark-style pipelines demonstrating compile-time failures per SchemaPolicy. Each test uses
 * PipelineBuilder with typed endpoints so the compiler must materialize SchemaConforms evidence, triggering
