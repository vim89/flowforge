 ...
 * RunnerWiringExample --------------------
 *
 * Example showing how to wire the SAME pipeline to different engines by swapping the DataAlgebra
 * implementation (Spark vs Flink), and how Kafka could be configured at the edges (simulated here for
 * portability).
 *
 * Run: sbt "examples/runMain com.flowforge.examples.runners.RunnerWiringExample --engine spark --input
 * examples-data/users.csv --output target/examples/spark-out" sbt "examples/runMain
 * com.flowforge.examples.runners.RunnerWiringExample --engine flink --input examples-data/users.csv --output
 * target/examples/flink-out"
 *
 * Notes:
...
object RunnerWiringExample {
  ...
  def runWithSpark(args: Args): Unit = {
    ...
    val sparkR: FlowforgeResource[IO, SparkSession] = FlowforgeResource.make(IO {
      SparkSession
        .builder().appName("RunnerWiringExample").master("local[*]")
    ...
    F.flatMap(validateNonEmpty("brokers", brokers)) { _ =>
      F.flatMap(validateNonEmpty("topicIn", topicIn)) { _ =>
        F.flatMap(validateNonEmpty("topicOut", topicOut)) { _ =>
          F.delay(
            println(
              s"[$label] Kafka configured: brokers=$brokers in=$topicIn out=$topicOut sr=$schemaRegistry",
            ),
          )
        }
      }
      ...
      rest match {
        ...
        case Nil =>
          for {
            ...
            i   <- in
            o   <- out
          } yield Args(eng, i, o, m.getOrElse("file"))
  ...
  def kafkaFlow[F[_]](
    ...
    def parseCsvUsers(path: String): F[List[User]] = F.blocking {
      ...
      try
        src.getLines().drop(1).toList.flatMap { line =>
          val parts = line.split(",", -1)
          if (parts.length >= 3)
      ...
      F.flatMap(F.delay(println(s"[$label-kafka] publishing ${users.size} records to $topicIn"))) { _ =>
        F.flatMap(in.publishAll(users)) { _ =>
          F.flatMap(in.consumeAll) { consumed =>
            ...
            F.flatMap(F.delay(println(s"[$label-kafka] consumed ${consumed.size} records from $topicIn"))) {
              _ =>
