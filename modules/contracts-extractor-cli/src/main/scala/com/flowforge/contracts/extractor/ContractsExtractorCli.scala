...
object ContractsExtractorCli extends IOApp.Simple {
  ...
  private val parser = {
    ...
    OParser.sequence(
      ...
      opt[String]("mode").required().action((m, a) => a.copy(mode = m.toLowerCase match {
      ...
      })).text("parquet | delta | hive | csv | jdbc"),
      opt[String]("input").required().action((p, a) => a.copy(input = p)).text("path, table, or identifier"),
      opt[String]("domain").optional().action((d, a) => a.copy(domain = d)).text("domain name"),
      opt[String]("entity").optional().action((e, a) => a.copy(entity = e)).text("entity name (record name)"),
      opt[String]("namespace").optional().action((n, a) => a.copy(namespace = n)).text("Avro namespace"),
      opt[String]("output").optional().action((o, a) => a.copy(outputDir = o)).text("output directory (contracts root)"),
      opt[String]("jdbc-url").optional().action((u, a) => a.copy(jdbcUrl = Some(u))).text("JDBC url (for mode=jdbc)"),
      opt[String]("jdbc-table").optional().action((t, a) => a.copy(jdbcTable = Some(t))).text("JDBC table (for mode=jdbc)"),
      opt[String]("master").optional().action((m, a) => a.copy(master = Some(m))).text("Spark master (default: local[*])")
    )
  }
  ...
  private def sparkResource(master: Option[String]): Resource[IO, SparkSession] =
    Resource.make {
      EffectSystem[IO].blocking {
        val b = SparkSession.builder().appName("ff-contract-extractor").config("spark.ui.enabled", "false")
        master.foreach(b.master)
        b.getOrCreate()
      }
    }(s => EffectSystem[IO].blocking(s.stop()).void)

  private def extract(args: Args): IO[Unit] = sparkResource(args.master).use { spark =>
    val df = args.mode match {
      ...
      case Mode.CSV     => spark.read.option("header", "true").option("inferSchema", "true").csv(args.input)
      case Mode.JDBC    =>
        val url = args.jdbcUrl.getOrElse(throw new IllegalArgumentException("--jdbc-url required for mode=jdbc"))
  ...
  object Avro {
    ...
    private def sparkTypeToAvro(dt: org.apache.spark.sql.types.DataType, nullable: Boolean): io.circe.Json = {
      ...
      val base: Json = dt match {
        ...
        case StructType(fields) =>
          val subFields = fields.toList.map(f => io.circe.Json.obj(
  ...
  object Files {
    ...
    def writeContracts(root: String, domain: String, entity: String, avroJson: String): IO[Unit] = IO {
    ...
    private def firstField(avroJson: String): Option[String] = {
      ...
      parse(avroJson).toOption.flatMap(_.hcursor.downField("fields").downArray.downField("name").as[String].toOption)
    }
  }
}
