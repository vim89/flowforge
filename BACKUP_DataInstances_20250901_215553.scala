...
object DataInstances {
  ...
  private def createSimpleMockDataAlgebra[F[_]: EffectSystem]: DataAlgebra[F] =
    new DataAlgebra[F] {
       ...
       */
      def read[A](source: DataSource)(implicit decoder: DataDecoder[A]): F[Dataset[A]] =
        F.delay(Dataset.empty[A])

      def readWithSchema[A](source: DataSource, expectedSchema: DataSchema)(implicit
        decoder: DataDecoder[A],
        validator: SchemaValidator[A]
      ): F[Either[FlowForgeError, Dataset[A]]] =
        F.delay(Right(Dataset.empty[A]))

      def stream[A](source: DataSource)(implicit decoder: DataDecoder[A]): F[DataStream[F, A]] =
        F.raiseError(new NotImplementedError("Streaming not implemented in mock"))

      def readBatch[A](source: DataSource, batchSize: Int)(implicit
        decoder: DataDecoder[A]
      ): F[List[Dataset[A]]] =
        F.delay(List(Dataset.empty[A]))

      def transform[A, B](dataset: Dataset[A], transformation: A => F[B])(implicit
        encoder: DataEncoder[B]
      ): F[Dataset[B]] =
        F.delay(Dataset.empty[B])

      def transformPipeline[A, B](dataset: Dataset[A], transformations: NonEmptyList[A => F[B]])(
      ...
      def mapWithEffect[A, B](dataset: Dataset[A], f: A => F[B])(implicit
        encoder: DataEncoder[B]
      ): F[Dataset[B]] =
        dataset.data
          .traverse(f)
      ...
      def flatMapWithEffect[A, B](dataset: Dataset[A], f: A => F[Dataset[B]])(implicit
        encoder: DataEncoder[B]
      ): F[Dataset[B]] =
        F.delay(Dataset.empty[B])

      def groupBy[A, K, V](dataset: Dataset[A], keyExtractor: A => K, aggregator: List[A] => V)(
        implicit encoder: DataEncoder[V]
      ): F[Dataset[(K, V)]] =
      ...
      def join[A, B, K, C](
        ...
        rightKey: B => K,
        combiner: (A, B) => C
      )(implicit encoder: DataEncoder[C]): F[Dataset[C]] =
      ...
      def evolveSchema[A, B](dataset: Dataset[A], migration: SchemaMigration[A, B])(implicit
        encoder: DataEncoder[B]
      ): F[Dataset[B]] =
        F.delay(Dataset.empty[B])
      ...
      def write[A](dataset: Dataset[A], sink: DataSink)(implicit
      ...
      def writeWithOptions[A](dataset: Dataset[A], sink: DataSink, options: WriteOptions)(implicit
        encoder: DataEncoder[A]
      ): F[WriteResult] =
        write(dataset, sink)

      def writeStream[A](stream: DataStream[F, A], sink: DataSink)(implicit
        encoder: DataEncoder[A]
      ): F[WriteResult] =
        F.raiseError(new NotImplementedError("Stream writing not implemented in mock"))

      def writeBatch[A](datasets: List[Dataset[A]], sink: DataSink)(implicit
        encoder: DataEncoder[A]
      ): F[List[WriteResult]] =
      ...
      // Utility Operations
      def count[A](dataset: Dataset[A]): F[Long]      = F.delay(dataset.size.toLong)
      def isEmpty[A](dataset: Dataset[A]): F[Boolean] = F.delay(dataset.isEmpty)
      ...
      def cache[A](dataset: Dataset[A], strategy: CacheStrategy): F[Dataset[A]] = F.delay(dataset)
      def partition[A](
        dataset: Dataset[A],
      ...
      def computeChangeHash[A](record: A, hashColumns: NonEmptyList[FieldName]): F[String] =
        F.delay("mock-hash")

      // Table Operations (implementing TableOperations trait methods)
      def repairRefreshTable(
        ...
        F.delay(
          TableOperations.TableOperationResult(
            ...
            recordsProcessed = 0L,
            processingTime =
              scala.concurrent.duration.FiniteDuration(100, scala.concurrent.duration.MILLISECONDS),
            errors = List.empty
          )
        )
        ...
        F.delay(
          TableOperations.TableOperationResult(
            tableName = TableOperations.TableName(
              eu.timepit.refined.refineMV("default"),
              eu.timepit.refined.refineMV("table")
            ),
            ...
            recordsProcessed = 0L,
            processingTime =
              scala.concurrent.duration.FiniteDuration(0, scala.concurrent.duration.MILLISECONDS),
            errors = List.empty
          )
        )
